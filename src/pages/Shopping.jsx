...
import { STAPLES, formatIngredient } from '../lib/meals-data'
import { getShoppingItems, addShoppingItem, updateShoppingItem, deleteShoppingItem, clearCheckedItems } from '../lib/data'
...
export default function ShoppingPage() {
  ...
  const loadData = async () => {
    try {
      const itemsData = await getShoppingItems()
      // Normalize field names
      setItems(itemsData.map(i => ({
        ...i,
        estimated_cost: i.estimatedCost || i.estimated_cost,
        added_by: i.addedBy || i.added_by,
      })))
    } catch (e) {
  ...
  const updateItemCost = async (id, cost) => {
    try {
      await updateShoppingItem(id, { estimatedCost: cost ? parseFloat(cost) : null })
      loadData()
    } catch (e) {
  ...
  const addStaplesCategory = async (category) => {
    ...
    try {
      ...
      console.error('Error adding staples:', e)
    }
  ...
  const addAllStaples = async () => {
    ...
    try {
      ...
      console.error('Error adding all staples:', e)
    }
    setAddingStaples(false)
    loadData()
  }
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-12">
        <div className="animate-pulse text-teal-400 text-xl">Loading shopping list...</div>
      </div>
    )
  }
          ...
          {uncheckedItems.length > 0 && (
            <div className="space-y-2">
              {uncheckedItems.map(item => {
                const addedBy = users.find(u => u.id === item.added_by || u.id === item.addedBy)
